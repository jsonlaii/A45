import {getMonsterFromRepository, updateMonsterInRepository, deleteMonsterFromRepository,createMonstersInRepository} from "../repositories/monster.repository.js"
...
export const getMonster = async (req,res)=> {
    ...
    try{
        ...
        res.status(500).send(e.message, `faied to fetch monster ${id}`);
    }
}
...
export const updateMonster = async (req, res) => {
    ...
    const {body} = req;
    try{
        const monster = await updateMonsterInRepository({_id:id}, body);
        res.status(200).send(monster)
    }catch(e){
...
export const createMonster = async(req,res) => {
    ...
    try{
        ...
        res.status(500).send(e.message, `failed to fetch monster`)
    }
}; 